...
def main() -> None:
    ...
    for ep in range(args.episodes):
        ...
        while not done:
            ...
            poses.append(obj)
            # A crude but honest occlusion flag: the hand is between the camera
            # (at -y) and the box, and close to it laterally.
    ...
    print("wrote {}: {} frames, {:.0f} MB, {:.1%} occluded".format(
        args.output, len(images), os.path.getsize(args.output) / 1e6, occluded.mean()))
